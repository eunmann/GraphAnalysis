...
#pragma omp parallel for schedule(static)
			for (size_t vertex = 0; vertex < graph.num_vertices(); vertex++) {
				...
				if (row_index_end - row_index >= 8) {
					...
					for (uint32_t riev = row_index_end - 8; row_index < riev; row_index += 8) {
						...
						__m256i neighbor_1_v = neighbor_v << 32;
						_mm256_insert_epi32(neighbor_1_v, graph.col_ind[neighbor_row_index_end], 7);
						__m256 adjacency_v = _mm256_set_ps(1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f);
						adjacency_v = adjacency_v / _mm256_cvtepi32_ps((neighbor_1_v - neighbor_v));
	...
	template<template<class> class alloc_type, template<class> class temp_alloc_type>
	void breadth_first_traversal(const GraphCRS<alloc_type>& graph, uint32_t vertex) {
		...
		while (!frontier_vec_1.empty() || !frontier_vec_2.empty()) {
			...
			std::vector<uint32_t, temp_alloc_type<uint32_t>>& frontier_r = level % 2 == 1 ? frontier_vec_1 : frontier_vec_2;
			std::vector<uint32_t, temp_alloc_type<uint32_t>>& frontier_w = level % 2 == 0 ? frontier_vec_1 : frontier_vec_2;
